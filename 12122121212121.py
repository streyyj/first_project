...
class FCardAutoMatchAndPlayMod(loader.Module):
    ...
    async def менюcmd(self, message):
        ...
        try:
            ...
            buttons_text = "\n".join(
                [f"{self.strings['debug_buttons'].format(button.text)}" for row in response.buttons for button in row]
            )
            await utils.answer(message, f"🪄 Кнопки:\n{buttons_text}")
            ...
            # Выводим все кнопки для отладки
            buttons_text = "\n".join(
                [f"{self.strings['debug_buttons'].format(button.text)}" for row in play_response.buttons for button in row]
            )
            await utils.answer(message, f"🪄 Кнопки после 'Матч':\n{buttons_text}")

            # Ищем кнопку "Играть матч"
            play_button = None
            for row in play_response.buttons:
                for button in row:
                    if isinstance(button, MessageButton) and button.text == "🎮 Играть матч":  # Здесь укажи правильный текст
                        play_button = button
                        break
                if play_button:
                    break
