...
class FCardAutoMatchAndRatingMod(loader.Module):
    ...
    async def менюcmd(self, message):
        ...
        try:
            ...
            # Шаг 3: Нажимаем кнопку "Играть матч"
            await play_button.click()
            await utils.answer(message, self.strings["clicking_play_button"])

            # Шаг 4: Ждём ответное сообщение с кнопкой "Рейтинг"
            rating_response = await self._client.get_messages(bot_username, limit=1)
            if not rating_response:
                return await utils.answer(message, "❌ Не получил ответ после нажатия 'Играть матч'")

            rating_response = rating_response[0]
            if not rating_response.buttons:
                return await utils.answer(message, "❌ В ответе нет кнопок после 'Играть матч'")

            # Ищем кнопку "Рейтинг"
            rating_button = None
            for row in rating_response.buttons:
                for button in row:
                    if isinstance(button, MessageButton) and button.text == "🏆 Рейтинг":
